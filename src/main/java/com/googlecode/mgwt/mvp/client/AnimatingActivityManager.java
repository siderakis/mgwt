...
/**
 * This is a fork of @link {@link ActivityManager} that has the same features, but also adds
 * animations to the lifecycle of Activities.
 * 
 * It can be used as a replacement for {@link ActivityManager}, but requires an instance of
 * {@link AnimationMapper} to work properly
 * 
 * @author Daniel Kurka
 * @version $Id: $
 */
public class AnimatingActivityManager implements PlaceChangeEvent.Handler,
   ...
   */
  public AnimatingActivityManager(ActivityMapper mapper, AnimationMapper animationMapper,
      EventBus eventBus) {
    this.mapper = mapper;
   ...
   * <p>
   * The current activity's widget will be hidden immediately, which can cause flicker if the next
   * activity provides its widget asynchronously. That can be minimized by decent caching.
  ...
  public void onPlaceChange(PlaceChangeEvent event) {
    ...
    if (startingNext) {
      ...
      try {
        ...
        /*
         * And kill them off again in case it was naughty and added new ones during onstop
         */
        stopperedEventBus.removeHandlers();
      }
    }
    ...
    /*
     * Now start the thing. Wrap the actual display with a per-call instance that protects the
     * display from canceled or stopped activities, and which maintains our startingNext state.
     */
    try {
  ...
  private native void stopTouchEventPropagation(boolean disable)/*-{
		var handler =@com.googlecode.mgwt.mvp.client.AnimatingActivityManager::stopPropagationEventHandler(Lcom/google/gwt/dom/client/NativeEvent;);
		var el = $wnd;
		if (disable) {
			...
			el.addEventListener("touchleave", handler, true);
			...
			el.removeEventListener("touchcancel", handler, true);
			el.removeEventListener("touchleave", handler, true);
			el.removeEventListener("touchmove", handler, true);
		}
  }-*/;

  private void updateHandlers(boolean activate) {
    if (activate) {
      final com.google.web.bindery.event.shared.HandlerRegistration placeReg =
          eventBus.addHandler(PlaceChangeEvent.TYPE, this);
      final com.google.web.bindery.event.shared.HandlerRegistration placeRequestReg =
